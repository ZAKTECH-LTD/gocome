            ...
            <div class="">
                ...
                <div class='grid grid-cols-1 gap-4 mb-10 md:grid-cols-2 '>
                    ...
                    <div class="block mt-4 text-sm">
                        ...
                        <p class="py-2 text-xs font-light"><span class="pr-2 text-sm font-medium text-red-600">Note:</span>{{ __('This needs to be abled for the rest driver wallet related settings to work') }}</p>
                    </div>
                    ...
                    <div class="block p-2 mt-4 text-sm border rounded">
                        ...
                        <x-checkbox title="{{ __('Enable') }}" name="driverWalletRequiredForTotal" :defer="true" />
                        <p class="py-2 text-xs font-light"><span class="pr-2 text-sm font-medium text-red-600">Note:</span>{{ __('Driver must have enough in wallet balance to cover order total or only admin commission from delivery fee') }}</p>
                    </div>
                    ...
                    <div class="block mt-4 text-sm">
                        ...
                        <p class="py-2 text-xs font-light"><span class="pr-2 text-sm font-medium text-red-600">Note:</span>{{ __('Irrespective of the order payment method, order delivery fee will have to be paid in cash to delivery personal') }}</p>
                    </div>
                ...
                <p class="text-xs italic">{{ __("Delivery info below will be use for vendor withut delivery info") }}</p>
                <div class='grid grid-cols-1 gap-4 mb-10 md:grid-cols-2 lg:grid-cols-3'>
                ...
                <div class='grid grid-cols-1 gap-4 mb-10 md:grid-cols-2 lg:grid-cols-3'>
                    <div class="block mt-4 text-sm">
                        <p>{{ __('Wallet') }}</p>
                        <x-checkbox title="{{ __('Enable') }}" description="{{ __('Allow wallet') }}" name="allowWallet" :defer="true" />
                    </div>
                ...
                @can('view-loyalty-settings')    
                    ...
                    <div class='grid grid-cols-1 gap-4 mb-10 md:grid-cols-2 lg:grid-cols-3'>
                        <x-input title="{{ __('Amount To 1 Point') }}" 
                            hint="{{ __('This is the amount that will be rewared with 1 point. e.g Set 0.001 if you want every') }} {{ setting('currency','$') }} {{ __('0.001 spent to be rewarded with 1 point') }}" name="amount_to_point" />
                        <x-input title="{{ __('1 Point To Amount') }}" 
                            hint="{{ __('This is the amount that 1 point can be converted to. e.g Set 0.0001 if you want every 1 point to be convertable to 0.001 and credited to user wallet') }}" name="point_to_amount" />
                        <div class="block mt-4 text-sm">
                            <p></p>
                        <x-checkbox title="{{ __('Enable') }}" description="{{ __('Enable Loyalty Point System') }}" name="enableLoyalty" :defer="true" />
                        </div>
                    </div>
                @endcan
